...
void HttpService::mgEvHandler(struct mg_connection *nc, int ev, void *p) {
    ...
    if (ev == MG_EV_HTTP_REQUEST) {
        ...
        //body内容
        char* pbody = new char[msg->body.len + 1];
		memset(pbody, 0, msg->body.len + 1);
		memcpy(pbody, msg->body.p, msg->body.len);
		printf("%s\n", pbody);
		...
		memset(uri, 0, msg->method.len + 1);
		memcpy(method, msg->method.p, msg->method.len);
		printf("%s\n", method);

        //返回body信息
        mgSendBody(nc, "content bbbbbb");
